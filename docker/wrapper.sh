...
declare dest1="$DESTINATION_PEER_ID_1"
if [ -z "$dest1" ]; then
    echo "DESTINATION_PEER_ID_1 is not set"
    exit 1
fi

declare dest2="$DESTINATION_PEER_ID_2"
if [ -z "$dest2" ]; then
    echo "DESTINATION_PEER_ID_2 is not set"
    exit 1
fi
...
awk -v cont="$dest1" '{gsub(/destinations."<peerid1>"/, "destinations." cont ); print}' config.toml >temp.toml && mv temp.toml config.toml
awk -v cont="$dest2" '{gsub(/destinations."<peerid2>"/, "destinations." cont ); print}' config.toml >temp.toml && mv temp.toml config.toml
