...
/// exit node via a different routing config does not reuse a pseudonym that was
/// issued for a different path, which would cause the exit node to reject it.
pub struct PseudonymCache {
    inner: HashMap<(Address, RoutingMode), (HoprPseudonym, Instant)>,
    ttl: Duration,
}
...
impl PseudonymCache {
    ...
    pub fn get(&mut self, dest: &Destination) -> Option<HoprPseudonym> {
        ...
        self.inner
            .get(&(dest.address, dest.routing.clone()))
            .map(|(pseudonym, _)| *pseudonym)
    }

    pub fn insert(&mut self, dest: &Destination, pseudonym: HoprPseudonym) {
        self.inner
            .insert((dest.address, dest.routing.clone()), (pseudonym, Instant::now()));
    }

    pub fn remove(&mut self, dest: &Destination) {
        self.inner.remove(&(dest.address, dest.routing.clone()));
    }
}
