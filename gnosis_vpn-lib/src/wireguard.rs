...
impl WireGuard {
    ...
    pub fn to_file_string(
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            lines.push("PostUp = ip -6 route add blackhole ::/1".to_string());
            lines.push("PostUp = ip -6 route add blackhole 8000::/1".to_string());
            lines.push("PreDown = ip -6 route del blackhole ::/1".to_string());
            lines.push("PreDown = ip -6 route del blackhole 8000::/1".to_string());
        }
        #[cfg(target_os = "macos")]
        {
