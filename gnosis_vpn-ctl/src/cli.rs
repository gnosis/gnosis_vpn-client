...
pub enum Command {
    ...
    /// Open a long-lived probe session to this exit and keep checking its version, latency and load
    #[command()]
    Probe {
        /// Endpoint node address
        id: String,
...
impl From<Command> for LibCommand {
    fn from(val: Command) -> Self {
        match val {
            ...
            Command::Probe { id } => LibCommand::Probe(id),
            Command::Unprobe {} => LibCommand::Unprobe,
            Command::QuickProbe { id } => LibCommand::QuickProbe(id),
...
pub fn generate_completions(shell: clap_complete::Shell) {
    ...
    match shell {
        clap_complete::Shell::Fish => {
            ...
            for subcommand in ["connect", "probe", "quickprobe"] {
                print!(
                    "\ncomplete -c gnosis_vpn-ctl \
                    -n '__fish_gnosis_vpn_ctl_using_subcommand {subcommand}' \
...
_gnosis_vpn-ctl() {{
    ...
    for (( i=1; i<${{#COMP_WORDS[@]}}; i++ )); do
        if [[ "${{COMP_WORDS[i]}}" == "connect" || "${{COMP_WORDS[i]}}" == "probe" || "${{COMP_WORDS[i]}}" == "quickprobe" ]]; then
            connect_index=$i
            break
        fi
    done
