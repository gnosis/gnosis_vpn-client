...
) -> Result<FallbackRouter, Error> {
    let _ = worker;
    Ok(FallbackRouter { wg_data, peer_ips })
}
...
impl Routing for FallbackRouter {
    async fn setup(&mut self) -> Result<(), Error> {
        ...
        let extra = build_fallback_extra_lines(&self.peer_ips, interface_gateway);
...
fn build_fallback_extra_lines(peer_ips: &[Ipv4Addr], interface_gateway: (String, Option<String>)) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn build_fallback_extra_lines_include_table_off() {
        let peer_ips = [Ipv4Addr::new(10, 0, 0, 1)];
        let interface_gateway = ("en0".to_string(), Some("192.168.88.1".to_string()));

        let extra = super::build_fallback_extra_lines(&peer_ips, interface_gateway);
    ...
    #[test]
    fn build_firewall_router_returns_fallback() {
        ...
        let router: super::FallbackRouter =
