...
//! 2. Brings up WireGuard natively (wireguard-go + `wg setconf`, no automatic routing)
...
struct StaticRouter {
    ...
    /// Resolved WireGuard interface name (e.g. "utun8"); populated after WireGuard up.
...
impl Routing for StaticRouter {
    ...
    ///
    /// Phase 2: native WireGuard up (wireguard-go + wg setconf, no automatic routing)
    ///   - On failure: rollback Phase 1 bypass routes
    ///
    /// Phase 3 (after WireGuard up): add VPN routes via the resolved utun interface
