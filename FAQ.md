...
IP, and IPv6 leak protection), fetch and run the connectivity smoke test after
connecting:

```
curl -fsSL -o vpn-smoke-test.sh https://raw.githubusercontent.com/gnosis/gnosis_vpn-testenv/main/scripts/vpn-smoke-test.sh
chmod +x vpn-smoke-test.sh
./vpn-smoke-test.sh
```

It needs only `bash`, `curl`, and `ping` (both bundled with Linux and macOS)
and exits non-zero if any check fails. Run `./vpn-smoke-test.sh --help` for
options.
