...
- bypassed traffic needs NAT masquerading (`-j MASQUERADE`) on the WAN interface so the upstream gateway can route responses back (packets otherwise retain the VPN subnet source IP)
