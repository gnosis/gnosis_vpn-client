...
# Usage: ./run_baseline.sh [EXIT ...]   (default: USA UK_1 India)
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/.." && pwd)"
CTL="$REPO/target/release/gnosis_vpn-ctl"
OBSCURA="$HERE/bin/obscura"
CDP="ws://127.0.0.1:9222/devtools/browser"

EXITS=("$@"); [ ${#EXITS[@]} -eq 0 ] && EXITS=(USA UK_1 India)
...
if ! curl -s --max-time 3 http://127.0.0.1:9222/json/version >/dev/null 2>&1; then
  ...
  STARTED_OBSCURA=$!
  for i in $(seq 1 20); do curl -s --max-time 2 http://127.0.0.1:9222/json/version >/dev/null 2>&1 && break; sleep 0.5; done
fi
...
wait_for() { # wait_for <fn> <timeout_s> <label>
  ...
  while [ "$i" -lt "$to" ]; do "$fn" && return 0; sleep 3; i=$((i+3)); done
  echo "!! timeout waiting for $label (${to}s)"; return 1
}
...
for EXIT in "${EXITS[@]}"; do
  ...
  # ensure clean slate
  if ! is_disconnected; then "$CTL" disconnect >/dev/null 2>&1; wait_for is_disconnected 60 "disconnect(pre)"; fi

  echo "-- connect $EXIT"
  "$CTL" connect "$EXIT"
  if ! wait_for is_connected 150 "connect $EXIT"; then
    echo "{\"exit\":\"$EXIT\",\"expected_cc\":\"$CC\",\"fatal_error\":\"connect timeout\"}" > "$OUTDIR/$EXIT.json"
    "$CTL" disconnect >/dev/null 2>&1; wait_for is_disconnected 60 "disconnect"; continue
  fi
  echo "-- connected; waiting for data path to egress via $CC (guard against local leak)"
  DP_OK=""; LASTLOC=""
  for i in $(seq 1 20); do # up to ~100s; tunnel needs time to actually route (esp. far exits)
    LASTLOC="$(curl -s --max-time 20 https://cloudflare.com/cdn-cgi/trace 2>/dev/null | sed -n 's/^loc=//p')"
    if [ "$LASTLOC" = "$CC" ]; then echo "-- data path up, egress=$CC (after ${i}x)"; DP_OK=1; break; fi
    sleep 3
  done
  if [ -z "$DP_OK" ]; then
    ...
    "$CTL" disconnect >/dev/null 2>&1; wait_for is_disconnected 60 "disconnect"; sleep 5; continue
  fi
  "$CTL" -o json nerd-stats > "$OUTDIR/$EXIT.nerdstats.json" 2>/dev/null || true
  sleep 2

  EXIT="$EXIT" OUTDIR="$OUTDIR" EXPECTED_CC="$CC" CDP="$CDP" node "$HERE/harness.mjs"
...
echo ""
echo "== aggregating =="
OUTDIR="$OUTDIR" EXIT_ORDER="$(IFS=,; echo "${EXITS[*]}")" node "$HERE/aggregate.mjs"

if [ -n "$STARTED_OBSCURA" ]; then echo "== stopping obscura serve ($STARTED_OBSCURA) =="; kill "$STARTED_OBSCURA" 2>/dev/null; fi
echo "== done -> $OUTDIR =="
